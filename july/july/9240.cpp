...
{
	pld v1 = p2v(point[0], p1);
	pld v2 = p2v(point[0], p2);
	ll res = ccw(v1, v2);
...
void solve()
{
	...
	{
		pll p1 = CH[li];
		pll p1_nxt = CH[(li + 1) % CH.size()];
		pll p2 = CH[ri];
		pll p2_nxt = CH[(ri + 1) % CH.size()];

		ld curDist = dist(p1, p2);
		if (mxDist < curDist)
		{
			mxDist = curDist;
			ans1 = li; ans2 = ri;
		...
		if (ccw(v1, v2) > 0) li = (li + 1) % CH.size();
		else ri = (ri + 1) % CH.size();
	}
	cout << fixed;
	cout.precision(6);
	cout << mxDist << "\n";
